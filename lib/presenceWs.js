...
class Presence extends EventEmitter {
  constructor(shard = config.shard || 's6', { synchronizeSelf = false } = {}) {
    super();
    this.shards = shardList(shard, config.shardFallbacks);
    this.shard = this.shards[0] || 's6';
    this.cookie = null;
    this.queueWs = null;
  ...
  async connect() {
    ...
    let lastError = null;
    for (const shard of this.shards.length ? this.shards : ['s6']) {
      this.shard = shard;
      try {
        await this._queue();
        return;
        ...
        try { if (this.presenceWs) this.presenceWs.terminate(); } catch {}
        this.presenceWs = null;
        if (!isShardRejectedError(error)) throw error;
        this.log(`shard ${shard} rejected by entry gate; trying fallback`);
      }
    }
    throw lastError || new Error('no shard could be entered');
  }
  ...
  _presence() {
    return new Promise((resolve, reject) => {
      ...
      ws.on('message', (buf) => {
        ...
        if (d.t === 'region_ack') this._onRegionAck(d);
  ...
  _onRegionAck(message) {
    const region = message?.region;
    const handoff = this._regionHandoff;
    if (handoff && Date.now() < handoff.until && region !== handoff.target) {
      this.log(`ignoring stale region_ack region=${region} target=${handoff.target}`);
      return;
    }
    if (region) this.region = region;
    this.emit('region_ack', message);
  }
  ...
  _onSnap(d) {
    ...
    if (Array.isArray(d.players) && this.myId != null) {
      ...
      if (self) {
        ...
        const handoff = this._regionHandoff;
        if (handoff && Date.now() >= handoff.until) this._regionHandoff = null;
        const staleDuringHandoff = handoff && Date.now() < handoff.until && region !== handoff.target;
        const regionChanged = !!(region && region !== this.region);
        if (!staleDuringHandoff && region && Number.isFinite(x) && Number.isFinite(z) && (!this._walking || regionChanged)) {
          this.region = region;
          this.pos = {
            x,
  ...
  setRegion(region, x, z, y = null) {
    const previous = this.region;
    this._regionHandoff = { previous, target: region, until: Date.now() + 6000 };
    this.region = region;
...
if (require.main === module) {
  const p = new Presence(process.argv[2] || config.shard || 's6');
