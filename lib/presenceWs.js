...
class Presence extends EventEmitter {
  constructor(shard = config.shard || 's4') {
    super();
  ...
  async connect() {
    if (this._closing) throw new Error('presence client is closing');
    ...
    for (let i = 0; i < this.shards.length; i++) {
      ...
      try {
        ...
        if (!isShardRejectedError(e) || i >= this.shards.length - 1) throw e;
        this.log(`shard ${this.shard} rejected with 403, trying ${this.shards[i + 1]}`);
      }
    }
    throw lastError || new Error('presence connection failed');
  }
