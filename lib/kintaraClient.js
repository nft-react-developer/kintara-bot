...
// Automatic re-login is limited to once per 15s to avoid storms during repeated server 401s.
...
class KintaraClient {
  ...
  /**
   * Log in through walletAuth (private key) -> set cookie + player.
   * De-dupe: parallel calls share one promise. Respect RELOGIN_MIN_GAP_MS.
  ...
  /** Factory: log in first, then return {client, player}. Replaces login()+new boilerplate in callers. */
  ...
  async _request(method, base, pathname, body = null, timeoutMs = DEFAULT_TIMEOUT, _retried = false, _502attempt = 0) {
    if (!this.cookie) {
      if (this.autoLogin) {
        ...
        throw new Error('KINTARA_SESSION_COOKIE is not set in .env — log in manually in the browser and copy the cookie, or use autoLogin/WALLET_PRIVATE_KEY.');
      }
    }
    ...
    try {
      ...
      if (!json) {
        ...
        // Non-JSON response (probably an HTML login page -> expired cookie)
        ...
        throw new Error(`Non-JSON response (status ${res.status}) — the cookie may be expired or invalid. Body: ${text.slice(0, 200)}`);
      }
      ...
      if (!res.ok) {
        ...
        // Automatically log in again once if the cookie expired (401) and autoLogin is enabled.
  ...
  async fanoutGetWithFallback(pathname, timeoutMs) {
    try {
      return await this.fanoutGet(pathname, timeoutMs);
    } catch {
      return this.get(pathname, timeoutMs);
    }
  }
  ...
  lootBag(bagIdOrPayload) {
    const body = typeof bagIdOrPayload === 'object' && bagIdOrPayload !== null
      ? { ...bagIdOrPayload }
      : { bagId: String(bagIdOrPayload) };
    return this.post('/api/wild/loot-bag', body, 30000);
  }
  wildDie(payload = {}) { return this.post('/api/wild/die', payload); }

  // ============ MARKETPLACE ============
  marketplaceListings({ limit = 20, offset = 0, itemType = null, mine = false, sort = null, currency = null, category = null, q = null } = {}) {
  ...
  merchantCampaign() { return this.fanoutGetWithFallback('/api/world/merchant-campaign'); }
