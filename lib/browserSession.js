// ============ BROWSER SESSION — drive game client asli ============
// Reusable: launch chromium (browser cached) + inject cookie kintara_session,
// buka /play, join server (semua FULL -> antri), tunggu admission ke dunia.
// Dipakai oleh tools/move-test.js, dan nanti farmer persisten.
const fs = require('fs');
...
function sessionCookie() {
  ...
  if (!m) throw new Error('KINTARA_SESSION_COOKIE tidak ada di .env');
...
/**
 * Buka /play, join server (klik card), tunggu sampai masuk dunia.
 * @param {import('playwright-core').Page} page
 * @param {(s:string)=>void} log
 * @param {number} maxMin batas tunggu antrian
 * @returns {Promise<boolean>} true kalau berhasil masuk dunia
 */
async function enterWorld(page, log = console.log, maxMin = 25) {
  ...
  while (Date.now() < deadline) {
    ...
    if (!st.bootVisible && !st.queueing) {
      ...
      if (ok) { log('✅ MASUK DUNIA (t+' + tick * 8 + 's)'); return true; }
    }
  }
  return false;
}

/** Ambil canvas gameplay utama (terbesar) untuk klik-to-move / koordinat. */
