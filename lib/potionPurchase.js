...
async function navigateToShop(presence, log = () => {}) {
  if (presence.region === 'alchemist_shop') {
    ...
    if (presence.region !== 'alchemist_shop') {
      log(`Activating alchemist shop portal spawn x=${SHOP.shopSpawn.x} z=${SHOP.shopSpawn.z}`);
      const confirmed = presence.waitForRegionConfirmation('alchemist_shop', 12000);
      presence.setRegion('alchemist_shop', SHOP.shopSpawn.x, SHOP.shopSpawn.z, 0.41);
      await confirmed;
    }
...
async function leaveShop(presence, log = () => {}) {
  ...
  if (presence.region !== 'world') {
    log(`Activating Mainland portal spawn x=${SHOP.worldEntrance.x} z=${SHOP.worldEntrance.z}`);
    const confirmed = presence.waitForRegionConfirmation('world', 12000);
    presence.setRegion('world', SHOP.worldEntrance.x, SHOP.worldEntrance.z, 0.25);
    await confirmed;
  }
