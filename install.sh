#!/usr/bin/env bash
# ============================================================
#  KINTARA BOT — one-liner installer
#  bash <(curl -fsSL https://raw.githubusercontent.com/rygroup-dev/kintara-bot/main/install.sh)
#
#  Non-interaktif:
#  WALLET_PRIVATE_KEY=xxx TELEGRAM_BOT_TOKEN=yyy bash <(curl -fsSL .../install.sh)
# ============================================================
set -euo pipefail

REPO="https://github.com/rygroup-dev/kintara-bot.git"
DIR="${KINTARA_DIR:-kintara-bot}"

echo "🤖 Kintara Bot installer"

# --- Node check ---
command -v node >/dev/null 2>&1 || { echo "❌ Node.js >=18 belum terpasang. Install dulu: https://nodejs.org"; exit 1; }
command -v git  >/dev/null 2>&1 || { echo "❌ git belum terpasang."; exit 1; }

# --- clone / update ---
if [ -d "$DIR/.git" ]; then
  echo "📂 update repo..."; git -C "$DIR" pull --ff-only
else
  echo "📥 clone repo..."; git clone --depth 1 "$REPO" "$DIR"
fi
cd "$DIR"

# --- deps ---
echo "📦 install dependencies..."
npm install --no-audit --no-fund --omit=optional

# --- .env (kalau belum ada) ---
if [ ! -f .env ]; then
  WK="${WALLET_PRIVATE_KEY:-}"
  TT="${TELEGRAM_BOT_TOKEN:-}"
  if [ -z "$WK" ]; then read -rsp "🔑 Solana WALLET_PRIVATE_KEY (base58, gak ditampilkan): " WK </dev/tty; echo; fi
  if [ -z "$TT" ]; then read -rp  "💬 TELEGRAM_BOT_TOKEN (dari @BotFather): " TT </dev/tty; fi
  cp .env.example .env
  # inject aman (pakai | sebagai delimiter; nilai base58/token gak ada |)
  sed -i "s|^WALLET_PRIVATE_KEY=.*|WALLET_PRIVATE_KEY=${WK}|" .env
  sed -i "s|^TELEGRAM_BOT_TOKEN=.*|TELEGRAM_BOT_TOKEN=${TT}|" .env
  chmod 600 .env
  echo "✅ .env dibuat (chmod 600, gitignored)."
else
  echo "ℹ️  .env sudah ada — dilewati."
fi

# --- start telegram control bot (background) ---
echo "🚀 start Telegram control bot..."
mkdir -p recon/control
nohup node tools/telegram-bot.js > recon/telegram.log 2>&1 &
echo $! > recon/control/telegram.pid
sleep 2
echo ""
echo "✅ SELESAI! Bot kontrol jalan (pid $(cat recon/control/telegram.pid))."
echo "   Buka Telegram bot kamu, ketik /start lalu /help."
echo "   Command: /fish /gather /mine /auto /stop /status /skills /balance /quest"
echo "   Log: $DIR/recon/telegram.log"
